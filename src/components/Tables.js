...
import {  Form, InputGroup,Col, Row, Nav, Card, Image, Button, Table, Dropdown, ProgressBar, Pagination, ButtonGroup } from '@themesberg/react-bootstrap';
...
export const TransactionsTable = (props) => {
  ...
  const submitSearch  = () =>{
    ...
    .then(function (response) {
      console.log(response.data.payload.ruleList);
      setRuleData(response.data.payload.ruleList)
    })
  ...
  const totalRuleData= ruleData.length;


  // console.log(props.ruleData)

  const TableRow = (props) => {
    const { ruleNameSpace, condition, action, priority, probability, description } = props;
    ...
    <Row className="justify-content-between align-items-center">
      <Col xs={8} md={6} lg={3} xl={4}>
        <InputGroup>
          ...
          <Form.Control onChange={(e)=>{setSearch(e.target.value)}}  value={search} type="text" placeholder="Search" />
          <Button onClick={submitSearch} variant="secondary" className="m-1">Search</Button>
        ...
        <Table hover className="user-table align-items-center">
          <thead>
            <tr>
              ...
              <th className="border-bottom">Rule</th>
              <th className="border-bottom">Condition</th>
              <th className="border-bottom">Action</th>
              <th className="border-bottom">Priority
              </th>
              <th className="border-bottom">Description</th>
              <th className="border-bottom">Probability</th>

              <th className="border-bottom">actions</th>
            </tr>
          </thead>
          <tbody>
            {ruleData.map(t => <TableRow key={`ruleData-${t.id}`} {...t} />)}
          </tbody>
        </Table>
        <Card.Footer className="px-3 border-0 d-lg-flex align-items-center justify-content-between">
          <Nav>
            <Pagination className="mb-2 mb-lg-0">
              ...
              <Pagination.Item onClick={()=>setPages("0")} active>1</Pagination.Item>
              <Pagination.Item onClick={()=>setPages("1")}>2</Pagination.Item>
              <Pagination.Item onClick={()=>setPages("2")}>3</Pagination.Item>
              <Pagination.Item onClick={()=>setPages("3")}>4</Pagination.Item>
              <Pagination.Item onClick={()=>setPages("4")}>5</Pagination.Item>
              <Pagination.Next>
                Next
              </Pagination.Next>
            </Pagination>
          ...
          <small className="fw-bold">
            Showing <b>{totalRuleData}</b> out of <b>25</b> entries
          </small>
        </Card.Footer>
      </Card.Body>
    </Card>
